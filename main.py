...
class Socks5Server(SocketServer.StreamRequestHandler):
    ...
    def buildSocksTrain(self):
        iplist = list(self.iplist)
        try:
            remote = socket.socket(socket.AF_INET, socket.SOCK_STREAM)
            remote.connect(iplist.pop(0))
        except socket.error:
            pass
        local = self.sockNext(remote,iplist)
        return remote,local
    def handle(self):
        fp = open('list')
        self.iplist = []
        for line in fp:
            ...
            self.iplist.append((addr,int(port)))
        try:
            ...
            try:
                if mode == 1:  # 1. Tcp connect
                    ...
                    print 'To', addr, port[0]
                    self.iplist.append((addr,port[0]))
                    remote,local = self.buildSocksTrain()
                    print local
    ...
    def sockNext(self, sock, iplist):
        ...
        if sock.recv(2) == '\x05\x00':
            ...
            if len(iplist) == 0:
                return socket.inet_ntoa(data[4:8]), struct.unpack('>H', data[8:10])[0]
            else:
